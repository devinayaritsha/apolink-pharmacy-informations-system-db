...
-- show patient product payment
select p.nama_pasien, pr.nama_produk
from apolink.pasien p
	join apolink.transaksi t on p.pasien_id = t.pasien_id
	join apolink.detail_transaksi dt on t.transaksi_id = dt.transaksi_id
...
select kategori, count (*) as jumlah_produk from apolink.produk group by kategori;

-- show total stock per category
select kategori, sum (stok) as total_stok from apolink.produk group by kategori;
...
-- show supplier receiving history
...
-- show supplier who supply product
select s.nama_supplier, p.nama_produk
from apolink.supplier s
	join apolink.penerimaan_barang pb on s.supplier_id = pb.supplier_id
	join apolink.detail_penerimaan dp on pb.penerimaan_id = dp.penerimaan_id
...
select so.tanggal_opname, p.nama_produk, dso.stok_sistem, dso.stok_fisik, dso.selisih, so.keterangan,
	case
		when dso.selisih = 0 then 'Sesuai'
		when dso.selisih > 0 then 'Kelebihan Stok'
		when dso.selisih < 0 then 'Kekurangan Stok'
	end as status_stok
...
order by so.tanggal_opname desc, p.nama_produk;
